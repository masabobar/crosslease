...
│   └── fixture.ts   # Placeholder — rename to test.ts and implement before authoring specs
...
├── specs/           # Playwright test specifications, one file per user story / AC group
│   └── spec.ts      # Placeholder — empty, create per-story .spec.ts files here
├── tests/           # Generated BDD test suites (Gherkin .md files) — output of manual-test-suite-generator skill
│   └── PRD1042-43 User Login.md   # Naming convention: "<ID> <Title>.md", one file per story
├── CLAUDE.e2e.md    # This file
└── playwright.config.ts
```

> **Status:** All subdirectories exist but contain only placeholder files. The first real file to create is `fixtures/test.ts` (rename/replace `fixture.ts`) — it re-exports (and extends) `test` and `expect` from `@playwright/test` and is the **only** permitted import source for specs and page objects. No spec can be authored until this file exists.
>
> The `tests/` directory holds Gherkin `.md` files generated by the `qa-lead` agent pipeline — human-readable BDD specs, not Playwright files. Playwright specs live in `specs/`.
...
| 4 | `manual-test-suite-generator` | Stage 1–3 output | Gherkin `.md` file saved to `tests/<ID> <Title>.md` |

Stage 2 and 3 are skipped automatically when no Figma URL is attached to the story. Stage 4 is blocked if Stage 3 produces a CRITICAL mismatch. Stories that fail DoR (missing ACs, Backlog/Draft status) are excluded before Stage 2.
