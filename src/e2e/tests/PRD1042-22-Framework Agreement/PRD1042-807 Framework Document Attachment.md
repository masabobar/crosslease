...
Updated per CR PRD1042-1495 (2026-07-24): Document Type is now OPTIONAL — "Uncategorized" is the default and acceptable value (A6). All FA documents are optional in the first place until the client confirms a required set; the mandatory-documents list is configurable per tenant, not hard-coded (B3). Design PNG shows the pre-CR required state.
...
| AC-CR-B3 | [CR B3] All FA documents optional in the first place; mandatory list configurable per tenant, not hard-coded    | `edge-case`        | Bundled into AC-01 activation-time invariant — activation does not require a specific document type; configuration-level (tenant flag) is not E2E-observable in this story (belongs to US 11.02 activation + tenant admin config screen) |
