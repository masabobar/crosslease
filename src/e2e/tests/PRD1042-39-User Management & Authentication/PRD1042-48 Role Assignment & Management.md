...
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------- |
| MAJOR    | Role assignment screen during user creation not present in Figma frame 396:18538 — only the Edit role & scope dialog on User Detail is designed; AC-01 scenarios derive from story ACs only                                                                 | AC-01        | Designer — provide Figma frame for the user creation form with role dropdown, tenant selector, and scope fields visible |
| MINOR    | No FO + BO/Risk conflict error state or message designed in the Edit role & scope dialog; story requires a validation message for this case                                                                                                                 | AC-10        | Designer — add a conflict validation error state to the Edit role & scope dialog for FO/BO mutual exclusivity           |
| MINOR    | Leasing Company assignment field not visible in the Edit role & scope dialog design; story requires this field for Leasing Company User role; unclear whether it appears conditionally when "Leasing Company User" is selected                              | AC-14        | Designer / BA — confirm whether the LC assignment field is conditionally shown and add to design if so                  |
| MINOR    | Role dropdown open/expanded state not designed — input shows a pre-selected "Support" badge but the full dropdown with all predefined values is not captured                                                                                                | AC-03        | Designer — provide the open/expanded state of the New role dropdown                                                     |
| MINOR    | Post-submission state of the Edit role & scope dialog not designed — unclear what the user sees after clicking "Submit for approval" (success toast, pending indicator on ROLE & SCOPE section, or redirect)                                                | AC-07        | Designer — add post-submission state (pending indicator or confirmation) to the dialog and the ROLE & SCOPE section     |
| INFO     | Open question (comment 35511, Philipp Maute): Auditor role classification proposed as tenant-level read-only rather than platform-level. If accepted this changes AC-13 assertions (role classification value shown in ROLE & SCOPE) and affects PRD1042-70 | AC-13        | BA / Philipp / Vesna — confirm Auditor classification and update PRD1042-48 AC-13 and PRD1042-70 before automation      |
| INFO     | AC-08 (permission update) and AC-09 (session impact) excluded from this suite as separate-feature. If session revalidation on role change is to be E2E tested it should be added to PRD1042-47 Session Management scope.                                    | AC-08, AC-09 | QA / BA — confirm whether session revalidation on role change is in scope for PRD1042-47 or a dedicated story           |
