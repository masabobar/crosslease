...
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| MAJOR    | Locked account state not designed on the login page — no visual design exists for the lockout message component (banner, blocked form, or error state)                                                                | AC-03 / AC-05 | Designer — add "locked account" error state to login page design (same Figma file as PRD1042-43 login frames)                   |
| MAJOR    | Pre-lockout generic error message and locked-account message not differentiated in any design frame — risk of inconsistent copy or visual treatment between the two states                                            | AC-07         | Designer — define both error states on the login page with confirmed copy for each; confirm they share the same generic styling |
| MAJOR    | Admin manual unlock action not designed — no unlock button, confirmation modal, or post-unlock state found in User Detail View design (PRD1042-73)                                                                    | AC-13         | Designer — add manual unlock button and confirmation dialog to User Detail View; link to PRD1042-73 Figma design                |
| MINOR    | Lockout auto-expiry (AC-06) excluded from E2E scenarios — requires time-manipulation fixture to be testable without real waiting                                                                                      | AC-06         | Dev team — provide clock-override or short-TTL lockout configuration fixture before AC-06 can be automated                      |
| MINOR    | Security event escalation (AC-11) has no confirmed admin UI surface — escalation flag visibility location is unspecified in this story                                                                                | AC-11         | BA — confirm where Power User roles see escalation flags, or raise a dedicated Security Events ticket                           |
| INFO     | Should the pre-lockout generic error and the locked-account message use identical UI components and copy? Identical treatment strengthens anti-enumeration but may make it harder for users to understand their state | AC-05 / AC-07 | BA / Designer — confirm copy strategy and visual differentiation (if any) for both error states                                 |
| INFO     | Manual unlock placement: standalone button on User Detail View, or a separate account lockout management screen? Confirms test navigation path for AC-13 automation                                                   | AC-13         | BA / Designer — confirm UI placement before automating AC-13 scenarios                                                          |
