...
description: PRD1042-72 US 28.5 User Search & Filtering — full pipeline complete, WARNINGS, 5 scenario blocks, Critical Quick Filter mismatch, Bank Admin added 2026-07-08
metadata:
  type: project
---

PRD1042-72 (US 28.5 | USER MANAGEMENT | User Search & Filtering) completed full pipeline 2026-06-02. Updated 2026-07-08 for Bank Admin role.

**Bank Admin update 2026-07-08** (per PRD1042-48 / Ivan Mladenovic decision 2026-07-06):

- Wire value `bank_admin`, user_type `bank_tenant`, tenant-scoped single-tenant admin
- Role filter dropdown now enumerates 7 values (was 6): Support User, System Admin, Bank Admin, Auditor, Front Office, Back Office/Risk, Leasing Company User
- Happy-path AC-01 Outline expanded 3→4 variants (added Bank Admin between System Admin and Support User)
- AC-04 governance-block scenario now runs as Bank Admin + asserts 7-value dropdown + asserts System Admin/Bank Admin remain distinct filter values
- AC-07 combined-filters scenario switched from generic "Power User / Admin" to Bank Admin + explicit 7-value dropdown enumeration + tenant-scope assertion
- AC-08 tenant isolation scenario re-anchored on Bank Admin (was generic "user scoped to tenant") — anchors on AC-08's explicit "Power User (Bank Admin) may search only within its own tenant" clause + added assertion that Tenant filter is Platform-only per Filter Visibility Matrix
- Jira status corrected in header: Dev in progress → UAT ready
