...
describe("cases schemas", () => {
  ...
  // A list row is no longer just a case with optional extras: the 14 Sep contract makes
  // `primary_entity` part of it, because the Status column has to say which of the three records
  // the status it is showing belongs to.
  const validListRow = { ...validCase, primary_entity: "case" }

  it("CaseListItemSchema is a case plus the list's own fields", () => {
    const parsed = CaseListItemSchema.parse(validListRow)
    expect(parsed.case_reference).toBe(validCase.case_reference)
    expect(parsed.primary_entity).toBe("case")
  })
  ...
  /**
   * The list row was a case plus six display fields the contract did not declare. The 14 Sep
   * contract declares them, renames two and adds five more, so this block now guards a different
   * thing: that the **required** ones really are required, and the ones the backend may still omit
   * really are optional.
   */
  describe("CaseListItemSchema", () => {
    const listRow = {
      ...validCase,
      ...
      rate_due_date: "2026-09-15",
      primary_entity: "refinancing_request",
    ...
    it("parses a full row", () => {
      expect(CaseListItemSchema.parse(listRow)).toEqual(listRow)
    })

    // `primary_entity` says which of the three records the Status column is reporting. A row
    // without it would render a status with no idea what it belongs to, so it is not optional.
    it("refuses a row with no primary_entity", () => {
      const withoutEntity: Record<string, unknown> = { ...listRow }
      delete withoutEntity.primary_entity
      expect(() => CaseListItemSchema.parse(withoutEntity)).toThrow()
    })

    // Both are enums on the wire now, so an unknown value fails at the boundary rather than
    // reaching a label lookup that would silently print it.
    it("refuses an origin or primary_entity outside the declared set", () => {
      expect(() =>
        CaseListItemSchema.parse({ ...listRow, origin: "carrier_pigeon" })
      ).toThrow()
      expect(() =>
        CaseListItemSchema.parse({ ...listRow, primary_entity: "contract" })
      ).toThrow()
    })

    it("defaults the two counts the backend may omit", () => {
      const withoutCounts: Record<string, unknown> = { ...listRow }
      delete withoutCounts.lessee_count
      delete withoutCounts.decision_round
      const parsed = CaseListItemSchema.parse(withoutCounts)
      expect(parsed.lessee_count).toBe(0)
      expect(parsed.decision_round).toBe(0)
    })

    // A backend that has the column but no value sends null, not an omitted key.
    it("accepts the display fields as explicitly null", () => {
      const parsed = CaseListItemSchema.parse({
        ...listRow,
        lc_partner_name: null,
        contract_count: null,
        phase: null,
        step_order: null,
        waiting_on_roles: null,
        rate_due_date: null,
        last_activity_at: null,
        last_activity_by_name: null,
      })
      expect(parsed.phase).toBeNull()
      expect(parsed.waiting_on_roles).toBeNull()
      expect(parsed.rate_due_date).toBeNull()
    })
  })

  it("parses a case list response", () => {
    const response = { items: [validListRow], total: 1 }
    expect(CaseListResponseSchema.parse(response).total).toBe(1)
    expect(CaseListResponseSchema.parse(response).items).toHaveLength(1)
  })
